...
export class ShopReviewService implements GeneralService<ShopReviewDto> {
  ...
  async create(dto: CreateShopReviewDto): Promise<ShopReviewDto> {
    try {
      ...
      const existingReview = await this.shopReviewRepository.findOne({
        where: { user, shop },
      });
      if (existingReview) {
      ...
      query.andWhere('shop_invoices.shop_id = :shopId', {
        shopId: shop.id,
      });
      ...
      if (!orderDetails) {
        throw new Error(
          'Please buy at least one product from this shop to review ',
        );
      }
