...
export class CouponService implements GeneralService<CouponDto> {
  ...
  async pagination(
    ...
    sort: string,
    order: 'ASC' | 'DESC',
    couponSearchFilterDto: CouponPaginationSearchFilter,
    withRelations = true,
    ...
    try {
      ...
      const query = await this.couponRepository.createQueryBuilder('coupon');
      if (withRelations) {
        query
          .leftJoinAndSelect('coupon.users', 'users')
          .leftJoinAndSelect('coupon.shops', 'shops')
          .leftJoinAndSelect('coupon.categories', 'categories')
          .leftJoinAndSelect('coupon.products', 'products')
          .leftJoinAndSelect('coupon.thanas', 'thanas')
          .leftJoinAndSelect('coupon.freeProduct', 'freeProduct')
          .leftJoinAndSelect(
            'coupon.freeProductAttribute',
            'freeProductAttribute',
          );
      }

      if (couponSearchFilterDto?.fromDate || couponSearchFilterDto?.toDate) {
        if (couponSearchFilterDto?.fromDate && couponSearchFilterDto?.toDate) {
          query.andWhere(
            'DATE(coupon.startDate) <= :endDate AND DATE(coupon.endDate) >= :startDate',
            {
              startDate: couponSearchFilterDto?.fromDate,
              endDate: couponSearchFilterDto?.toDate,
            },
          );
        } else if (couponSearchFilterDto?.fromDate) {
          query.andWhere('DATE(coupon.endDate) >= :fromDate', {
            fromDate: couponSearchFilterDto?.fromDate,
          });
        } else if (couponSearchFilterDto?.toDate) {
          query.andWhere('DATE(coupon.startDate) <= :endDate', {
            endDate: couponSearchFilterDto?.toDate,
          });
        }
      }

      if (couponSearchFilterDto.couponCode) {
        query.andWhere('lower(coupon.couponCode) like :couponCode', {
          couponCode: `%${couponSearchFilterDto?.couponCode.toLowerCase()}%`,
        });
      }
