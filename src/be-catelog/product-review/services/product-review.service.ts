...
export class ProductReviewService implements GeneralService<ProductReviewDto> {
  ...
  async create(dto: CreateProductReviewDto): Promise<ProductReviewDto> {
    try {
      ...
      // const currentUser = this.requestService.userSession();
      ...
      query.leftJoinAndSelect(
        'shop_invoices.shopInvoiceDetails',
        'shopInvoiceDetails',
      );
      query.where('shopInvoiceDetails.product_id = :productId', {
        productId: product.id,
      });
      query.leftJoinAndSelect('shop_invoices.order', 'order');
      ...
      query.andWhere('deliveryAssignment.status = :status', {
        status: ShippingStatus.DELIVERED,
      });

      const orderDetails = await query.getOne();

      if (!orderDetails) {
        throw new Error('You must buy this product to review');
      }
