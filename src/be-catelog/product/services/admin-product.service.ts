...
@Injectable()
export class AdminProductService implements GeneralService<ProductDto> {
  ...
  async findAll(): Promise<ProductDto[]> {
    try {
      const products = await this.productRepository.find({
        ...
        relations: ['shop', 'category', 'productAttributes'],
      });
  ...
  async stock(
    ...
    try {
      ...
      if (
        productSearchDto.merchantId &&
        productSearchDto.merchantId.length > 0
      ) {
        query.andWhere('merchants.id = :merchantId', {
          merchantId: productSearchDto.merchantId,
        });
      }
  ...
  async remove(id: string): Promise<DeleteDto> {
    try {
      ...
      const deleted = await this.productRepository.save({
        ...saveDto,
        isDeleted: ActiveStatus.enabled,
      });
    ...
    try {
      this.findAll()
        .then((products) => {
          ...
          this.searchClient
            .send(
              { service: 'products', cmd: 'post', method: 'rebase' },
              products,
            )
            .subscribe();
        })
