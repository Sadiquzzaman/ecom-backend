...
@Injectable()
export class AdminShippingService implements GeneralService<ShipmentDto> {
  constructor(
    ...
    try {
      ...
      query
        ...
        .andWhere('shop_invoices.assignStatus = :assignStatus', {
          assignStatus: shipmentAssignmentDeliveryStatusDto.assignStatus,
        });
      ...
      if (shipmentAssignmentDeliveryStatusDto.assignStatus == '1') {
        query
          .leftJoinAndSelect(
        ...
        if (shipmentAssignmentDeliveryStatusDto.shippingStatus) {
          query.andWhere('deliveryAssignment.status = :deliveryStatus', {
            deliveryStatus: shipmentAssignmentDeliveryStatusDto.shippingStatus,
          });
        }

        if (shipmentAssignmentDeliveryStatusDto.transporterId) {
          query.andWhere('deliveryAssignment.transporter = :transporterId', {
            transporterId: shipmentAssignmentDeliveryStatusDto.transporterId,
          });
        }
      }
  ...
  async getAssignedShopInvoicesForTransporter(
    ...
    try {
      ...
      query
        ...
        .leftJoinAndSelect('shipment-delivery-assignment.customer', 'customers')
  ...
  // getCustomerById = async (id: string): Promise<CustomerEntity> => {
