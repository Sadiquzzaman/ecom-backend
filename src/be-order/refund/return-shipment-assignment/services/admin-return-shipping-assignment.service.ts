...
@Injectable()
export class AdminReturnShippingAssignmentService
  implements GeneralService<RefundApprovalDto>
{
  constructor(
    ...
    private readonly refundApprovalDetailsRepository: Repository<RefundApprovalDetailsEntity>,
  ...
  async findApprovedDetailsById(
    id: string,
  ): Promise<RefundApprovalDetailsDto[]> {
    try {
      const query = this.refundApprovalDetailsRepository.createQueryBuilder(
        'refund_approval_details',
      );

      query
        .leftJoinAndSelect('refund_approval_details.product', 'products')
        .leftJoinAndSelect(
          'refund_approval_details.refundApproval',
          'refundApproval',
        )
        .leftJoinAndSelect(
          'refund_approval_details.productAttributes',
          'productAttributes',
        );

      query.where('refundApproval.id = :approvalId', { approvalId: id });

      const refundApproval = await query.getMany();
      return this.conversionService.toDtos<
        RefundApprovalDetailsEntity,
        RefundApprovalDetailsDto
      >(refundApproval);
  ...
  async getApprovedDetails(id: string): Promise<RefundApprovalDetailsEntity> {
    const refundApprovalDetails =
      await this.refundApprovalDetailsRepository.findOne({
        where: {
          id,
          ...isActive,
        },
    ...
    this.exceptionService.notFound(
      refundApprovalDetails,
      'Refund Approval Details Not Found!!',
    );
