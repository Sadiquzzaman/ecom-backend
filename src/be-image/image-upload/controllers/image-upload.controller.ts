...
export class ImageUploadController {
  ...
  @Post('shop-type')
  async uploadedFileShopType(@Body() redisImageDto: RedisImageDto, @Res() res) {
  ...
  async uploadFileShopTypeRedis(@UploadedFile() file, @Res() res) {
  ...
  @HttpCode(HttpStatus.CREATED)
  @Post('withdrawal-document')
  async uploadedFileWithdrawal(
    @Body() redisImageDto: RedisImageDto,
    @Res() res,
  ) {
    const file = await this.redisService
  ...
  @HttpCode(HttpStatus.OK)
  @Post('withdrawal-document-redis')
  @UseInterceptors(
    FileInterceptor('image', {
      storage: storage,
      fileFilter: imageFileFilter,
  ...
  async uploadFileWithdrawalRedis(@UploadedFile() file, @Res() res) {
    await this.redisService
      .getClient(Redis.REDIS_TMP_FILE)
      .set(file.filename, file.path);
