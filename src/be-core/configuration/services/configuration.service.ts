...
export class ConfigurationService implements GeneralService<ConfigurationDto> {
  ...
  async update(id: string, dto: ConfigurationDto): Promise<ConfigurationDto> {
    try {
      ...
      const updatedConfiguration = await this.configurationRepository.save(
        dtoToEntity,
        {
          reload: true,
        },
  ...
  async findTermsAndConditionByTitle(): Promise<ConfigurationDto> {
    const configuration = await this.configurationRepository.findOne({
      where: {
        ...isActive,
  ...
  async findPrivacyPolicyByTitle(): Promise<ConfigurationDto> {
    const configuration = await this.configurationRepository.findOne({
      where: {
        ...isActive,
  ...
  async findReturnAndRefundbyTitle(): Promise<ConfigurationDto> {
    const configuration = await this.configurationRepository.findOne({
      where: {
        ...isActive,
  ...
  async findAboutUs(): Promise<ConfigurationDto> {
    const configuration = await this.configurationRepository.findOne({
      where: {
        ...isActive,
        name: 'About Us',
